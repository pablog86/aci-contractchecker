...
def debug(obj, msj="debug msj" ,level=5):
    if _debug >= level:
...
#------------------------------------
    ...
    for line in table:
        ...
        printt ("{:<4} {:<{}} {:<{}} {:<14} {:<7} {:<{}} {:<15} ({:02d}){:<{}} ({}){:<{}}".format(
            iD, sPcTag,lenSource, dPcTag,lenDest, direction, operSt, scopeId,lenVRF, action, priorities[prio],prio,lenPrio, fltId,fltName,lenFilter
            ))
    printt("\n")
...
def get_method (url, query_target=None, target_subtree_class=None, query_target_filter=None, page=0):
    token = apic_login()
    try:
        response = requests.get(url,
            ...
            verify=verify_https)
        debug(response.status_code, "Debug output CODE {} -> (url={}, query_target={}, target-subtree-class={}, query-target-filter={}, page-size={}, page={}): ".format(
            inspect.stack()[1][3], url, query_target, target_subtree_class, query_target_filter, page_size, page),
            1)
        if response.status_code == requests.codes.ok:
            debug(response.json(), "Debug output {} -> (url={}, query_target={}, target-subtree-class={}, query-target-filter={}, page-size={}, page={}): ".format(
                inspect.stack()[1][3],url, query_target, target_subtree_class, query_target_filter, page_size, page
                ))
...
def get_filterid (filterdn):  
    ...
    response = get_method(url, query_target="children", target_subtree_class="vzRsRFltPOwner")
    if response != None:
        aux=response.json()["imdata"]
        i=1
        while len(aux) < int(response.json()["totalCount"]):
...
def get_zoningrule (pod_id, node_id, query=None, subtree=None, filters=None):  #TODO: paginacion (?subscription=yes&order-by=actrlRule.name|desc&page=0)
    url=APIC_URL+"/api/node/class/topology/pod-{}/node-{}/actrlRule.json".format(pod_id, node_id)
    response = get_method(url, query_target=query, target_subtree_class=subtree, query_target_filter=filters)
    if response != None:
        aux=response.json()["imdata"]
        i=1
        while len(aux) < int(response.json()["totalCount"]):
...
#Get contract info
def get_contracts_info(tenant, contract, query=None, subtree=None, filters=None) ->  dict:
    url=APIC_URL+"/api/node/mo/uni/tn-{}/brc-{}.json".format(tenant, contract)
    response = get_method(url, query_target=query, target_subtree_class=subtree, query_target_filter=filters)
    if response != None:
        aux=response.json()["imdata"]
        i=1
...
def get_subject_info(tenant, contract, subject, query=None, filters=None) ->  dict:
    url=APIC_URL+"/api/node/mo/uni/tn-{}/brc-{}/subj-{}.json".format(tenant, contract, subject)
    response = get_method(url, query_target="children", target_subtree_class="vzRsSubjFiltAtt", query_target_filter=filters)
    if response != None:
        aux=response.json()["imdata"]
        i=1
        while len(aux) < int(response.json()["totalCount"]):
    ...
    for epg in epgs:
        ...
        try:
            ...
            if d_vrfs[epg[obj_id]["attributes"][ctx_id]] in d_epgs:
                d_epgs[d_vrfs[epg[obj_id]["attributes"][ctx_id]]].update({epg[obj_id]["attributes"]["pcTag"]:epg[obj_id]["attributes"][epg_id]})
            else: 
                d_epgs[d_vrfs[epg[obj_id]["attributes"][ctx_id]]]={epg[obj_id]["attributes"]["pcTag"]:epg[obj_id]["attributes"][epg_id]}
            ...
            printt ("Undef scope:{} -> epg: {} ".format(epg[obj_id]["attributes"][ctx_id],epg[obj_id]["attributes"][epg_id]))
...
def get_vrf (filters=None) -> dict:
    global cache_vrf
    if len(cache_vrf) == 0 or filters != None :
        ...
        for vrf in vrfs:
            ...
            d_vrfs.update({"{}-pctag".format(vrf[obj_id]["attributes"][ctx_id]):vrf[obj_id]["attributes"][pctag]})
...
def EPGs (filters=None) -> dict:
    ...
    filter_type = {"ctx":"ctxDefDn", "epg":"epgPKey", "scope":"scopeId", "bd":"dn"}
    ...
    if filters:
        ...
        for epg_t in epg_type:
            aux = mapping_epg_pctag(epg_t, "wcard({}.{}, \"{}\")".format(epg_t, filter_type[f], filters))
            if aux != None:
                update(d_epgs, aux)
    else:
        for epg_t in epg_type:
...
def get_contract (tenant, contract):
    ...
    for c in get_contracts_info(tenant,contract,"children","vzSubj"):
        ...
        for s in get_subject_info(tenant, contract, c["vzSubj"]["attributes"]["name"]):
            d_contract["Subjects"][c["vzSubj"]["attributes"]["dn"]].append(s["vzRsSubjFiltAtt"]["attributes"]["tDn"])
    return d_contract

#---------------------------------------------------------------------------------------------------------------------------------------------

# def mapping_zoningrule_contract (pod_id, node_id, tenant, contract):
#     rule_type = ("id", "sPcTag", "dPcTag", "fltId", "direction", "operSt", "scopeId", "action", "prio")
#     rules={}

#     if tenant == None or contract == None:                          #All filters in the switch
#         filters = get_zoningrule (pod_id, node_id, "children")      #vzObservableRuleOwner
#         zoningrules = get_zoningrule (pod_id, node_id)
#         d_contract={}
#         if filters != []:
#             for f in filters:
#                 try :
#                     rule = "/".join(f["vzObservableRuleOwner"]["attributes"]["dn"].split("/")[0:7])
#                 except KeyError:
#                     rule = "/".join(f["vzRuleOwner"]["attributes"]["dn"].split("/")[0:7])
#                 for zoningrule in zoningrules:
#                     if zoningrule["actrlRule"]["attributes"]["dn"] == rule:
#                         break
#                 rules.update({rule:{}})
#                 for t in rule_type:
#                     rules[rule].update({t:zoningrule["actrlRule"]["attributes"][t]})
#                 try:
#                     rules[rule].update({"fltName":(re.findall("(?<=rsrFltAtt-\[).+?(?=\])", f["vzObservableRuleOwner"]["attributes"]["creatorDn"])[0])})
#                 except KeyError:
#                     if zoningrule["actrlRule"]["attributes"]["ctrctName"] != "":
#                         rules[rule].update({"fltName":zoningrule["actrlRule"]["attributes"]["ctrctName"]})
#                     else:
#                         rules[rule].update({"fltName":zoningrule["actrlRule"]["attributes"]["fltId"]})
#         else:
#             debug("No info vzObservableRuleOwner",level=1)
#             for zoningrule in zoningrules:
...
#                     else:
#                         rules[rule].update({"fltName":zoningrule["actrlRule"]["attributes"]["fltId"]})
#         d_contract.update({"rules/pod-{}/node-{}".format(pod_id, node_id):rules})

#     else:                                                           #Filters matching the tenant/contract
#         d_contract = get_contract(tenant, contract) 
#         if not bool(d_contract):
#             return None
#         filters = get_zoningrule (pod_id, node_id, "children", "vzObservableRuleOwner", "wcard(vzObservableRuleOwner.creatorDn,\"uni/tn-{}/brc-{}\")".format(
#             tenant, contract))
#         if filters != []:
#             for f in filters:
#                 rule = "/".join(f["vzObservableRuleOwner"]["attributes"]["dn"].split("/")[0:7])
#                 zoningrule = get_zoningrule(pod_id, node_id, filters="eq(actrlRule.dn,\"{}\")".format(rule))
#                 rules.update({rule:{}})
#                 for t in rule_type:
#                     rules[rule].update({t:zoningrule[0]["actrlRule"]["attributes"][t]})
#                 rules[rule].update({"fltName":(re.findall("(?<=rsrFltAtt-\[).+?(?=\])", f["vzObservableRuleOwner"]["attributes"]["creatorDn"])[0])})
#             d_contract.update({"rules/pod-{}/node-{}".format(pod_id, node_id):rules})
#         else:
#             debug("No info vzObservableRuleOwner",level=1)
#             zoningrules = get_zoningrule (pod_id, node_id, filters="wcard(actrlRule.ctrctName,\"{}:{}\")".format(tenant, contract))
#             #
#             if zoningrules != []:
#                 for zoningrule in zoningrules:
#                     rule = zoningrule["actrlRule"]["attributes"]["dn"]
...
#                     d_contract.update({"rules/pod-{}/node-{}".format(pod_id, node_id):rules})
#             else:
#                 d_contract.update({"rules/pod-{}/node-{}".format(pod_id, node_id):{}})
#     return d_contract
...
def contract_rules (pod_id, node_id, tenant=None, contract=None):   #prettify
    ...
    scopes= set(scopes)
    if tenant == None or contract == None:
        update(d_epgs, EPGs())
    else:
        for scope in scopes:
    ...
    for i in d_contract[node]:
        ...
        if d_contract[node][i]["sPcTag"]!= "any":
            try:
                ...
                printt("Key not found scopeId={} sPcTag={}".format(d_contract[node][i]["scopeId"],d_contract[node][i]["sPcTag"]))
        if d_contract[node][i]["dPcTag"]!= "any":
            try:
                ...
                printt("Key not found scopeId={} dPcTag={}".format(d_contract[node][i]["scopeId"],d_contract[node][i]["dPcTag"]))
...
-d 3 = Verbose"""
    ...
    indent_formatter = lambda prog: argparse.RawTextHelpFormatter(prog, max_help_position=50)
    parser = argparse.ArgumentParser(prog = "contract-checker", description=desc, formatter_class=indent_formatter,epilog=separator) 

    parser.add_argument('pod', metavar='podID',help='Pod ID number, eg: 1', type=int)
    parser.add_argument('node', metavar='nodeID',help='Node ID number, eg: 101', type=int)
    parser.add_argument('-t', '--tenant', action='store', help='Optional argument: Tenant of the contract to filter', metavar='Tenant Name')
    parser.add_argument('-c', '--contract', action='store', help='Optional argument: contract to filter', metavar='Contract Name')
    parser.add_argument('-d', '--debug', action='store', help=debugmsg, metavar='debug', type=int)
    parser.add_argument('-l', '--logfile', action='store_true', help='Optional argument: log in a file')
    ...
    try:
        if args.tenant == None and args.contract == None:
            printable (contract_rules (args.pod, args.node))
        else:
            printable (contract_rules (args.pod, args.node, args.tenant, args.contract))
